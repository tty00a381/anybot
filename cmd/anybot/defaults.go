...
命令会更新外部插件注册代码，并在 ` + "`plugins.d/<name>.yaml`" + ` 中加入默认禁用的插件配置项；可用 ` + "`module@version`" + ` 或 ` + "`-version`" + ` 固定版本，本地开发时可加 ` + "`-replace ../path/to/plugin`" + `。` + "`-replace`" + ` 按当前命令目录解析，并写成相对宿主目录的路径。确认配置后用 ` + "`anybot plugin enable`" + ` 启用插件，` + "`anybot plugin inspect`" + ` 查看当前配置和默认配置，` + "`anybot plugin check`" + ` 检查可加载插件配置，` + "`anybot up`" + ` 会构建宿主、同步并检查插件配置，然后运行。要调整外部插件版本、导出符号或本地替换路径，可使用 ` + "`anybot plugin update <name>`" + `，它不会改写插件配置。` + "`anybot build/up`" + ` 也会固定生成宿主对 AnyBot 自身的依赖，避免追随不确定的 ` + "`latest`" + `。移除外部插件可使用 ` + "`anybot plugin remove <name>`" + `。
`
