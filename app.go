...
func (a *App) Run(ctx context.Context) error {
	...
	if err := runHooks(ctx, a.startHooks); err != nil {
	...
	if a.workers <= 0 {
		if err := runHooks(ctx, a.readyHooks); err != nil {
			return fmt.Errorf("ready hook: %w", err)
		}
		return a.adapter.Start(ctx, a.dispatch)
	}
	...
	err := a.adapter.Start(ctx, func(eventCtx context.Context, event *Event) error {
		select {
